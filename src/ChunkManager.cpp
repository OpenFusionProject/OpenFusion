...
void ChunkManager::addNPC(int posX, int posY,int mapNum, int32_t id) {
    std::tuple<int, int, int> pos = grabChunk(posX, posY, mapNum);
...
void ChunkManager::addPlayer(int posX, int posY,int mapNum, CNSocket* sock) {
    std::tuple<int, int, int> pos = grabChunk(posX, posY,mapNum);
...
std::tuple<int, int, int> ChunkManager::grabChunk(int posX, int posY, int mapNum) {
    return std::make_tuple(posX / (settings::CHUNKSIZE / 3), posY / (settings::CHUNKSIZE / 3), mapNum);
}
...
std::vector<Chunk*> ChunkManager::grabChunks(std::tuple<int, int, int> chunk) {
    ...
    for (int i = -1; i < 2; i++) {
        for (int z = -1; z < 2; z++) {
            std::tuple<int, int, int> pos(std::get<0>(chunk) +i, std::get<1>(chunk) +z, std::get<2>(chunk));
...
bool ChunkManager::inPopulatedChunks(int posX, int posY, int mapNum) {
    auto chunk = ChunkManager::grabChunk(posX, posY, mapNum);
