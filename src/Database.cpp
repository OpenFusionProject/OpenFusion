...
void Database::open() {
	...
	if (file) {
		...
		DEBUGLOG(
			std::cout << "Database in operation" << std::endl;
		)
	}
	else {
...
void Database::addAccount(std::string login, std::string password) {
	sqlite3pp::command cmd(
		db, "INSERT INTO Accounts (Login, Password) VALUES (:login, :password)"
	);
...
void Database::finishCharacter(sP_CL2LS_REQ_CHAR_CREATE* character) {
	...
	sqlite3pp::command cmd(
		db, "UPDATE Players SET CharData = :data WHERE PlayerID = :id"
...
int Database::deleteCharacter(int characterID, int accountID) {
	...
	if (result>0) {
		...
		//delete
		sqlite3pp::command cmd(
			db, "DELETE FROM Players WHERE PlayerID = :ID"
		);
